...
- **Stage 5f outcome (2026-07-07): H14 VOID — validity gate failed 3/3, and the void is the finding.** No backbone re-solved its OWN training task with fresh latents (plumbing separately verified; tasks solved during joint training itself), so the shared transformation weights alone do not carry task knowledge — it lives in the latent/weight co-adaptation. Descriptive numbers (not decision-bearing): median jret/cold 3.436 (warm slower than cold on all 9), retention 6/9, probes 0/3. Delta-geometry readout: per-task adaptation deltas from the shared backbone are pairwise orthogonal (median cosine 0.0067) — no shared direction exists to amortize. 5090 cold rebase clean (9/11, same tasks as A40). Three independent weight-space-memory negatives now stand (H9 averaging, H10 retrieval, H14 joint basin); the memory thesis redirects to decomposed/concept-level forms (Stage 5e; ArcMemo) and the symbolic library route (Stage 1; Pang). H14b's trigger (marginal/kill) did not fire and its premise is undercut by the orthogonality result; owner fork recorded: run H14b anyway ($0 formality) vs close the weight-space track and return to Stage 6 (TTT-as-MDL) / Stage 2 (proposer). Recommendation: close.
...
- **Stage 6 outcome (2026-07-20): H15 KILL per the pre-registered rule.** Arm A 3/39 vs arm B 2/39; union-of-B oracle ties A, so even a perfect selector only equals longer convergence — no compute-allocation free lunch — and the MDL tail-loss selector is refuted (accuracy 1/2, loss ANTI-correlated with solving, echoing H10). The finding that outlives the verdict: **substrate coverage is seed-stochastic** — the identical H8 protocol on new hardware/seed solved 3 tasks H8 didn't, a seed-1 re-check lost 2/9 banked solves, and re-rolls grew the dev union 11 -> 15/50. All solve-rate claims are henceforth seed-marginalized. Remaining live slots: Stage 2 (proposer redefinition), substrate-external memory (symbolic library / concept memory); a voting/consistency selector across seeds is a deferred H16 candidate.
