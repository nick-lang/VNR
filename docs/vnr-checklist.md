...
- [~] H16 — augmentation-voting selection (Stage 6b; owner approved 2026-08-19): pre-registered in `ledger/hypotheses.md` (K=4 D8+color-perm frames x 1000 steps, reverse-mapped agreement voting; ACCEPT >= 13/50 voted pass@2, KILL <= 11/50; wrong-consensus vs E1-B's 2/6 as the sharp secondary). Transform selftest passed (40 round-trips exact). Equivariance/diversity GATE PASSED 2026-08-19 (non-identity frames solvable in-frame; distinct reverse-mapped answers; core task solved in all 4 frames with reverse-mapped agreement — the equivariance mechanism observed directly). Full 200-job queue LAUNCHED (~37 h remaining, $0, banked); `h16_eval.py` ready (pre-registered voting rule; dry-run clean on partial artifacts). `h16_runner.py`.
...
- [x] Write-up draft v1 (`writeup/draft-v0.md`, 2026-08-19): ALL experimental slots filled with E1-B + 232-run coverage numbers; abstract/contributions updated to run-stochasticity with same-config flip evidence. Remaining before submission: figures (union timeline, per-task p^), Kaggle ARC-AGI-2 linkage entry, arXiv packaging.
- [ ] Publication plan (2026-08-18): (1) arXiv preprint as soon as E1-B slots fill (priority timestamp on the run-stochasticity finding); (2) ARC Prize 2026 Paper Prize, DEADLINE Nov 2, 2026 — REQUIRES a linked Kaggle code submission on the ARC-AGI-2 or ARC-AGI-3 track ("need not score highly"); package CompressARC + validated selection stack (or tiny-LPN if H17 approved) as the linked entry; artifacts open-sourced within 7 days of deadline; (3) TMLR afterward (deadline-free, negative-results-friendly). NeurIPS 2026 workshops (~Aug 29 calls) = stretch only if E1-B lands this week.
...
- [ ] H18 design inputs from the owner's teatime discovery dossier (2026-08-19, reviewed + citations verified; notes in `lit/`): (a) MVG curriculum arm — Kashtan & Alon 2005 predicts reusable structure forms iff the training stream varies MODULARLY (shared subgoals); schedule re-arc generators as fixed vs shuffled vs modularly-varying and measure abstraction reuse + held-out compositional transfer (also the retroactive frame for H14's orthogonal deltas: random goals -> no shared structure); (b) library curation by Minton-style utility accounting (retain an abstraction only if measured savings x frequency > match cost) — the 35-year-old answer to "help vs bloat," with CBR case-base-maintenance policies as the playbook. Grammatical-inference learnability bounds (dossier direction 3) tracked as long-horizon theory only.
- Context: LPN (NeurIPS 2025) = the correctly-run inversion of our H14 negative; see `lit/2025-latent-program-network.md`.
- [x] H20 — MVG rescue attempt: **INCONCLUSIVE per pre-registered rule; symbolic MVG CLOSED** (2026-08-19). Confounds removed (capacity contest, EMA retention, depth-capped transfer after two pre-run assay probes): M >= R in 5/5 seeds — H19's kill confirmed as retention artifact — but effect only +2.0pp pooled (bar +10pp) under a 43pp assay ceiling; library-vs-raw +39pp. Closure per the no-third-variant commitment; MVG survives only as a candidate H18 arm (SGD substrate, lowered prior). By-product: the utility-contest library is a validated curation mechanism for H18. See notebook `2026-08-19-h20-mvg-rescue.md`.
- [x] H19 — MVG-curriculum pilot: **KILL per pre-registered rule** (2026-08-19; M/R >= 1.14 in 5/5 seeds, pooled 1.25; solve rate 150/150 all arms; library-vs-raw ~2.6x sanity holds). Mechanism finding: the SHUFFLED arm wins because 2-epoch utility-forgetting prunes macros the modular curriculum needs again — policy-curriculum coupling (forgetting horizon must exceed goal-return period). Any H18 MVG arm must pre-register that coupling; K&A import survives as theory only. See notebook `2026-08-19-h19-mvg-pilot.md`.
