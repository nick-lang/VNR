...
- [~] Write-up draft v0 (`writeup/draft-v0.md`, 2026-08-18): full structure + all banked numbers in; `[E1B: ...]` slots await capture results.
...
- [~] H19 — MVG-curriculum pilot on the Stage-1 symbolic substrate (CPU-only, $0): pre-registered 2026-08-19 in `ledger/hypotheses.md`; runner built + smoke passed (`experiments/poc-vnr-s9-mvg/h19_runner.py`); arm R is an exact permutation control of arm M (same 120 tasks, order-only difference); library mechanics unify MVG curriculum + Minton utility forgetting. QUEUED to launch when the E1-B GPU queue drains; full run ~minutes (5 seeds).
