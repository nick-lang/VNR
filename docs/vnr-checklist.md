...
- [~] H16 — augmentation-voting selection (Stage 6b; owner approved 2026-08-19): pre-registered in `ledger/hypotheses.md` (K=4 D8+color-perm frames x 1000 steps, reverse-mapped agreement voting; ACCEPT >= 13/50 voted pass@2, KILL <= 11/50; wrong-consensus vs E1-B's 2/6 as the sharp secondary). Transform selftest passed (40 round-trips exact). Equivariance/diversity GATE running first (~1.6 h, 8 jobs, reused by the full run); full queue (~39 h, $0, banked) launches only on gate PASS. `h16_runner.py`.
