...
- **Cost baseline for H9+:** mean ~52 min/task on L40S (range 25-128 min), 43.3 GPU-hours total, ~$84.50 list (~$1.69/task). Actual out-of-pocket lower after plan credits; the run survived a billing-cap pause mid-flight (21 results banked, 29 resumed — no recompute).
