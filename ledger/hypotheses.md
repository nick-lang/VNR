...
## H20 — MVG rescue attempt: capacity pressure + horizon-matched retention + hard transfer (Stage 9 pilot, second and final symbolic test)

### H20 pre-registration (2026-08-19, before running)

- **What H19 killed vs what it confounded (recorded before this run):** H19 killed temporal blocking under a retention policy whose forgetting horizon (2 epochs) was shorter than the goal-return period (~5-6 epochs) and under a capacity cap that never bound (no selection pressure — the K&A ingredient we omitted). It also could not discriminate library quality because every arm solved every transfer task (metric was noisy node counts). H20 removes all three named confounds AT ONCE, deliberately: this is a strongest-reasonable-version rescue, not a factor decomposition.
- **Hypothesis:** with binding capacity and horizon-matched score-based retention, a modularly varying stream grows a library that solves MORE held-out HARD compositions than the same tasks in random order.
- **Design (`experiments/poc-vnr-s9-mvg/h20_runner.py`):** stream identical in form to H19 (12 epochs x 10 two-concept tasks; arm M ring order / arm R exact permutation / arm F fixed pair; 5 seeds). Changes: (1) library capacity K=4 top-level macros, ALWAYS binding via top-K score selection at each epoch end over incumbents + new BPE proposals; (2) retention score = EMA with decay 0.8/epoch of solution-use counts (a macro used 2 epochs per 6-epoch cycle survives; one-epoch junk decays out) — proposals enter scored by their window bigram count; (3) transfer = 30 THREE-concept tasks from the held-out distance-2 pairs, largely unsolvable by raw primitives at the 50k budget, so the primary metric becomes SOLVE RATE (pass@train-consistency within budget), with nodes secondary; raw-primitives baseline reported.
- **Primary decision rule (pre-committed):** ACCEPT if pooled M transfer solve rate >= R + 10pp AND M >= R in >= 4/5 seeds. KILL if pooled M <= R. INCONCLUSIVE otherwise.
- **Closure rule (pre-committed):** a second kill CLOSES the MVG question for the symbolic/batch-compression substrate (two-kill convention, as with H9+H10). Any further MVG work then lives only as a pre-registered arm of H18 in an order-sensitive (SGD) learner, or not at all. No third symbolic variant.
- **Secondary readouts (not decision-bearing):** post-switch adaptation cost (mean nodes on the first 3 tasks after each M goal switch vs matched stream positions in R); library composition over time under capacity contest; alignment; F-arm overfit check; raw-baseline transfer solve rate (design validity: must be materially below library arms, else the hard-transfer premise failed and the run is VOID for the primary rule).
- **Cost gate:** CPU-only, ~10-30 min total, $0.

### H20 pre-run design amendment (2026-08-19, BEFORE the full run; assay-sensitivity probes)
- Two validity probes ran before the decision run (banked in the notebook): (1) at the original budget-hardness design, raw search solves 20/20 k=3 and 16/16 k=4 transfer tasks — tiny grids + the DSL's group structure collapse deep compositions into short equivalents, so budget alone cannot create hardness; (2) node-distribution probe: even a PERFECT 6-concept oracle library beats raw by only ~10-13pp at the best budget (median 675 vs 987 nodes) — BELOW the pre-registered 10pp accept bar, i.e. the assay as designed could not detect the effect it was registered to detect.
- **Amendment:** transfer evaluation is DEPTH-CAPPED at max_depth=3 (stream solving unchanged at 10; budget unchanged at 50k; symmetric across arms and the raw baseline). Probe at this setting: raw 17/30 (57%) vs oracle library 30/30 (100%) — assay ceiling ~43pp, comfortably above the accept bar. Side finding recorded: >half of 6-primitive compositions in this DSL have <=3-primitive equivalents (measured group collapse).
- Decision rule, arms, seeds, and closure rule are UNCHANGED.

### H20 outcome (2026-08-19): INCONCLUSIVE per the pre-registered rule — artifact confirmed, effect negligible; symbolic MVG closes per the no-third-variant commitment
- **Run:** 5 seeds x 3 arms, depth-capped transfer (max_depth 3), CPU, ~2 min, $0. Design-validity gate passed: raw baseline 54% pooled, far below the library arms.
- **Decision inputs:** pooled transfer solve rate M 0.933 vs R 0.913 (+2.0pp; accept needed +10pp) with M >= R in 5/5 seeds; not a kill (M > R), not an accept (effect tiny). INCONCLUSIVE.
- **What it settles:** (1) H19's M-worse verdict WAS the retention artifact — with capacity contest + EMA retention, the modular order is never worse and directionally better in every seed; (2) the artifact-free MVG effect in a batch-compression library is ~2pp — real direction, negligible size. Mechanism: BPE proposal statistics are order-invariant, so both orders converge to near-identical libraries (alignment readouts match); temporal structure can only act through retention dynamics, and a sane retention policy equalizes the arms. (3) The library-vs-raw value at depth-capped transfer is large (+39pp) — the utility-problem-informed contest library works; it is the CURRICULUM that adds almost nothing here.
- **Closure (per the H20 pre-registration's "no third symbolic variant"):** the MVG question is CLOSED for the symbolic/batch-compression substrate — not by two kills but by one artifact-contaminated kill plus one artifact-free null-to-tiny result under a 43pp assay ceiling. MVG survives solely as a candidate pre-registered arm of H18 in an order-sensitive (SGD) learner, where the K&A mechanism (structure shaped by the ORDER of selection pressure) actually exists; it carries a lowered prior and the retention-horizon lesson from H19.
- **Assay-sensitivity method note (reusable):** both pre-run probes (budget-hardness impossible; oracle-library ceiling below the accept bar; depth-cap restores a 43pp ceiling) were run BEFORE the decision run — measuring the assay's ceiling against the pre-registered effect size is now standing practice for future pilots.

## H16 — Augmentation-voting selection: input diversity + reverse-mapped agreement converts coverage into pass@2 (Stage 6b)

### H16 pre-registration (2026-08-19, before any GPU run; owner approved direction, bars drafted by the assistant per house pattern)

- **Hypothesis:** solving K=4 exactly-transformed variants of each task (D8 x color-permutation input augmentations) and voting over the REVERSE-MAPPED top picks yields more pass@2 solves on the committed 50-task dev split than any single run of the substrate — because truth is equivariant under the transforms while systematic errors mostly are not, so agreement-on-truth survives reverse-mapping and agreement-on-error decorrelates. (Sharpest falsifiable sub-prediction, from E1-B's measured flaw: the wrong-consensus rate on never-solved tasks drops below same-input voting's 2/6.)
- **Augmentation set (fixed):** aug0 = identity; aug1 = rot90; aug2 = flip_h; aug3 = transpose; aug1-3 each composed with a color permutation of colors 1-9 (0/background fixed), seeded deterministically per (task, aug). Same training seed (0) for all runs; 1000 steps; unmodified CompressARC on the transformed task.
- **Voting rule (fixed in advance):** candidate pool = the 4 reverse-mapped top-1 grids, ranked by (vote count, then summed within-run vote margin); answer 1 = first; answer 2 = second distinct top-1 if any, else the modal reverse-mapped top-2 (same tie-break). pass@2 = ground truth in {answer 1, answer 2}.
- **Primary decision rule (pre-committed):** voted pass@2 on the 50-task dev split: ACCEPT if >= 13/50 (strictly above the best single run ever banked, 11, + the program's customary +2 margin); KILL if <= 11/50 (at 4x1000 = 2x the standard 2000-step compute, failing to beat the best single run kills the mechanism); INCONCLUSIVE at 12.
- **Honest-accounting caveats (recorded in advance):** (1) 4x1000 steps is 2x the H8 protocol compute; the accept bar is set against the best-ever single run rather than a matched-compute control because H15 already established 1x2000 >= 2x1000 for singles; if H16 ACCEPTS, a seed-voting control at identical K and compute becomes the mandatory follow-up to attribute the gain to the AUGMENTATION axis rather than to voting per se. (2) Run-stochasticity means the voted result is itself one draw; the per-frame solve records are banked so the claim can be stated run-marginalized.
- **Equivariance/diversity GATE (pre-committed, runs first, ~1.6 h):** 2 tasks (1 core: `1d0a4b61`, 1 fringe: `73182012`) x 4 augs. (a) CPU selftest before any GPU: transform round-trip on grids and ground truths is exact; (b) VALIDITY: at least one non-identity variant of the core task solves in its frame (transformed tasks are well-posed for the substrate); (c) DIVERSITY: >= 2 distinct reverse-mapped top-1 answers on >= 1 of the 2 tasks (if all frames return identical answers, the substrate is effectively equivariant, augmentation degenerates to re-rolls, and H16 is VOID without the full spend, inheriting H15's kill). Gate jobs are reused by the full run (identical job spec).
- **Secondary readouts (not decision-bearing):** wrong-consensus rate on the 35 never-solved tasks vs E1-B's same-input 2/6; per-frame solve counts (does augmentation change per-frame solvability?); vote-margin/stability tie-break usage; union-of-frames oracle vs voted result (selector efficiency).
- **Cost gate:** 200 jobs x 1000 steps = 200k steps ~= 39 h wall at 3-way on the 5090, $0, banked/resumable; gate ~1.6 h of that, spent first.
